...
> 本文档系统性地分析了分布式存储系统中五种主流元数据管理架构模式，涵盖架构原理、代表性系统、优缺点分析以及行业演进趋势。
...
3FS（Fast and Flexible File System）是清华大学开发的面向 AI 训练场景的分布式文件系统：

| 特性 | 说明 |
|------|------|
| **元数据引擎** | 自研分布式元数据服务，支持多副本和自动分片 |
| **数据层** | 对象存储（支持 S3 协议） |
| **一致性模型** | 元数据强一致（基于 Raft），数据最终一致 |
| **针对 AI 优化** | 小文件聚合、大文件高吞吐读取、检查点快速写入 |
| **与 JuiceFS 差异** | 自研元数据引擎 vs 依赖外部数据库；更侧重 AI 场景的读写模式 |
